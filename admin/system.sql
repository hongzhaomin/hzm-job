...
(
    ...
    `token_version` bigint(20)  DEFAULT 0 COMMENT 'token版本号',
    `valid`         tinyint(2) NOT NULL DEFAULT 1 COMMENT '是否可用：1-可用；0-不可用',
...
INSERT INTO `hzm_user` (`user_name`, `password`, `role`, `email`, `token_version`)
VALUES ('admin', 'c4ca4238a0b923820dcc509a6f75849b', 0, '2318448951@qq.com', 0);
...
(
    `id`          bigint(20) unsigned NOT NULL AUTO_INCREMENT,
    `executor_id` bigint(20) NOT NULL COMMENT '执行器id',
    `address`     varchar(128) DEFAULT NULL COMMENT '节点地址',
    `status`      tinyint(2) DEFAULT 0 COMMENT '节点状态：0-离线；1-在线',
    `valid`       tinyint(2) DEFAULT 1 COMMENT '是否可用：1-可用；0-不可用',
...
(
    ...
    `status`                tinyint(2) DEFAULT 0 COMMENT '任务调度日志状态：0-待调度；1-任务执行中；2-任务结束',
...
-- 任务调度统计表
CREATE TABLE `hzm_schedule_statistics`
(
    `id`          bigint(20) unsigned NOT NULL AUTO_INCREMENT,
    `day`         date     DEFAULT NULL COMMENT '统计日期',
    `total_num`   bigint(20) DEFAULT 0 COMMENT '调度总数',
    `success_num` bigint(20) DEFAULT 0 COMMENT '调度成功数',
    `fail_num`    bigint(20) DEFAULT 0 COMMENT '调度失败数',
    `valid`       tinyint(2) DEFAULT 1 COMMENT '是否可用：1-可用；0-不可用',
    `create_time` datetime DEFAULT CURRENT_TIMESTAMP COMMENT '创建时间',
    `update_time` datetime DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP COMMENT '更新时间',
    PRIMARY KEY (`id`),
    KEY           `idx_day` (`day`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='任务调度统计表';

-- 操作日志表
CREATE TABLE `hzm_operate_log`
(
    `id`           bigint(20) unsigned NOT NULL AUTO_INCREMENT,
    `operator_id`  bigint(20) DEFAULT NULL COMMENT '操作人id',
    `description`  varchar(512) DEFAULT NULL COMMENT '操作描述',
    `detail`       text         DEFAULT NULL COMMENT '操作内容详情：字符串数组json',
